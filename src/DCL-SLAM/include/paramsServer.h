...
class paramsServer
{
	...
	protected:
		...
		bool startup_multi_frame_enable_; // force time-spaced keyframes while stationary at startup
		int startup_keyframe_count_; // number of startup keyframes to collect
		int startup_accumulation_frames_; // frames accumulated into each startup descriptor
		float startup_keyframe_interval_; // minimum seconds between forced startup keyframes
		...
		int iris_min_valid_column_bins_; // occupied range bins required for a valid MID360 azimuth column
		float iris_min_valid_overlap_ratio_; // valid azimuth overlap required for MID360 yaw estimation
		...
		float inter_robot_max_correspondence_distance_; // ICP correspondence distance for inter-robot matching
		float inter_robot_fine_correspondence_distance_; // fine ICP correspondence distance
		float inter_robot_yaw_search_range_deg_; // yaw search radius around descriptor estimate (degree)
		float inter_robot_yaw_search_step_deg_; // yaw hypothesis interval (degree)
		float inter_robot_overlap_distance_; // maximum nearest-neighbor distance used by overlap validation
		float inter_robot_min_overlap_ratio_; // minimum bidirectional overlap ratio
		int inter_robot_consistency_count_; // independent estimates required before adding an inter-robot factor
		float inter_robot_consistency_translation_threshold_; // translation agreement threshold (m)
		float inter_robot_consistency_rotation_threshold_; // rotation agreement threshold (rad)
		float inter_robot_translation_variance_; // startup inter-robot factor translation variance (m^2)
		float inter_robot_rotation_variance_; // startup inter-robot factor rotation variance (rad^2)
		float world_to_odom_max_translation_jump_; // maximum accepted update after global alignment is latched (m)
		float world_to_odom_max_rotation_jump_; // maximum accepted update after global alignment is latched (rad)
