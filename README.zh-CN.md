...
项目同时包含 ROS 1 和 ROS 2，环境加载顺序较为敏感。在不同终端分别启动组件时，请以 `scripts/run_robot.sh` 中的环境设置和进程顺序为准。如果经常需要分组件运行，建议为脚本增加 `--skip-bridge`、`--skip-livox` 等明确选项，不建议通过反复注释代码段来切换功能。

## 引用与致谢

本仓库基于 DCL-SLAM、FAST-LIO2、LIO-SAM 和 DOOR-SLAM 等开源项目构建。DCL-SLAM 论文引用和上游致谢见 [`ros1_ws/src/DCL-SLAM/README.md`](ros1_ws/src/DCL-SLAM/README.md)。各组件继续遵循其源码目录内的许可证与归属文件；仓库级归属说明见 [NOTICE](NOTICE)。

## 许可证

本仓库新增内容采用 [Apache License 2.0](LICENSE)。随仓库分发的第三方组件保留各自原始许可证，请查阅对应源码目录内的许可证文件。
