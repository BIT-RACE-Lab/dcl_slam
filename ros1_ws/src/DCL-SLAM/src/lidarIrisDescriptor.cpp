...
lidar_iris_descriptor::lidar_iris_descriptor(
	...
	int min_valid_column_bins = 2,
	float min_valid_overlap_ratio = 0.25,
	int nscale					= 4,
	...
	min_valid_column_bins_(std::max(1, min_valid_column_bins)),
	min_valid_overlap_ratio_(std::max(0.05f,
		std::min(min_valid_overlap_ratio, 1.0f))),
	...
	const pcl::PointCloud<pcl::PointXYZI> &cloud)
{
	...
	if(n_scan_ == 4) // MID-360: horizontal 360 deg, vertical -7 to 52 deg
	{
		constexpr float min_elevation_deg = -7.0f;
		constexpr float max_elevation_deg = 52.0f;
		constexpr float elevation_bin_deg =
			(max_elevation_deg - min_elevation_deg) / 8.0f;
		for(const auto& p : cloud.points)
		{
			const float dis = std::hypot(p.data[0], p.data[1]);
			const float elevation = std::atan2(p.data[2], dis) * 180.0f / M_PI;
			if(elevation < min_elevation_deg || elevation > max_elevation_deg)
			{
				continue;
			}
			const float yaw = std::atan2(p.data[1], p.data[0]) * 180.0f / M_PI + 180.0f;
			const int Q_dis = std::min(std::max(static_cast<int>(std::floor(dis)), 0), rows_ - 1);
			const int Q_arc = std::min(7, std::max(0,
				static_cast<int>(std::floor(
					(elevation - min_elevation_deg) / elevation_bin_deg))));
			const int Q_yaw = std::min(std::max(
				static_cast<int>(std::floor(yaw + 0.5f)), 0), columns_ - 1);
	...
	else if(n_scan_ == 6) // legacy Livox/Avia projection
	{
	...
	const cv::Mat1b &src)
{
	...
	desc.V = cv::Mat1b::ones(src.rows, src.cols) * 255;
	if(n_scan_ == 4)
	{
		// An empty MID-360 column is unknown/no-return data, not positive evidence
		// that two places have the same geometry. A column is valid only when it
		// contains several occupied radial cells. Replicate that visibility mask
		// into the log-Gabor mask so Hamming comparison ignores invalid sectors.
		for(int col = 0; col < src.cols; ++col)
		{
			if(cv::countNonZero(src.col(col)) < min_valid_column_bins_)
			{
				desc.V.col(col).setTo(0);
				desc.M.col(col).setTo(255);
			}
		}
	}
	return desc;
...
int lidar_iris_descriptor::estimateMaskedYawShift(
	const lidar_iris_descriptor::featureDesc &img1,
	const lidar_iris_descriptor::featureDesc &img2)
{
	int best_shift = -1;
	double best_distance = std::numeric_limits<double>::infinity();
	const int minimum_valid_cells = static_cast<int>(std::ceil(
		img1.img.rows * img1.img.cols * min_valid_overlap_ratio_));

	// Search the complete circle. Unlike FFT on a zero-filled image, the score
	// only uses cells observed by both scans after applying the candidate shift.
	for(int shift = 0; shift < img1.img.cols; ++shift)
	{
		const cv::Mat1b shifted_img1 = circShift(img1.img, 0, shift);
		const cv::Mat1b shifted_valid1 = circShift(img1.V, 0, shift);
		const cv::Mat1b common_valid = shifted_valid1 & img2.V;
		const int valid_cells = cv::countNonZero(common_valid);
		if(valid_cells < minimum_valid_cells)
		{
			continue;
		}

		const double different_bits = cv::norm(
			shifted_img1, img2.img, cv::NORM_HAMMING, common_valid);
		const double normalized_distance = different_bits /
			(static_cast<double>(valid_cells) * 8.0);
	...
	int *bias)
{
	if(n_scan_ == 4)
	{
		const int masked_shift = estimateMaskedYawShift(img1, img2);
		if(masked_shift < 0)
		{
			if(bias)
			{
		...
		int refined_shift;
		getHammingDistance(
			img1.T, img1.M, img2.T, img2.M,
			masked_shift, distance, refined_shift);
		if(bias)
		{
			*bias = refined_shift;
		}
	...
	// Reject unsupported modes explicitly instead of falling through with an
	// undefined return value.  This also keeps descriptor matching fail-safe if
	// a malformed runtime parameter is supplied.
	if(bias)
	{
		*bias = 0;
	}
...
void lidar_iris_descriptor::save(
	...
	const int index)
{
	// A latched descriptor can be delivered again after a subscriber reconnects.
	// Keep the global descriptor index one-to-one with (robot, keyframe).
	if(indexes_maps[robot].find(index) != indexes_maps[robot].end())
	{
		return;
	}
	...
	const int cur_ptr)
{
	...
	for(int i = 0; i < robot_num_; i++)
	{
		...
		{
			// Iterate one keyed container and look up the associated data.  The
			// iteration order of separate unordered_maps is not a public contract.
			for (const auto& indexes_map : indexes_maps[i])
			{
				const int frame_index = indexes_map.first;
				int cur_row = new_iris_rowkeys.cols();
				new_iris_rowkeys.conservativeResize(rows_, cur_row + 1);
				new_iris_rowkeys.block(0, cur_row, rows_, 1) =
					iris_rowkeys[i].at(frame_index).block(0, 0, rows_, 1);
				new_indexes_maps.emplace_back(indexes_map.second);
				new_iris_features.emplace_back(iris_features[i].at(frame_index));
			}
		}

		const int query_candidates = std::min(candidates_num_, static_cast<int>(new_indexes_maps.size()));
		if(query_candidates > 0)
		{
			kdTree = Nabo::NNSearchF::createKDTreeLinearHeap(new_iris_rowkeys, rows_);

			// search n nearest neighbors
			Eigen::VectorXi indice(query_candidates);
			Eigen::VectorXf distance(query_candidates);

			// Use every available candidate during startup, then cap the query at
			// candidates_num_.  This allows one descriptor from each stationary
			// robot to trigger geometric verification.
			// The query descriptor and this search cloud belong to different
			// robots.  Nabo otherwise treats a zero-distance cross-robot match as
			// a forbidden self match and also rejects a one-point search cloud.
			kdTree->knn(iris_rowkey, indice, distance, query_candidates, 0,
				Nabo::NNSearchF::ALLOW_SELF_MATCH);

			// step 2: pairwise distance
			// #pragma omp parallel for num_threads(4)
			for(int candidate_idx = 0; candidate_idx < query_candidates; candidate_idx++)
			{
				if(indice[candidate_idx] < 0 ||
					static_cast<size_t>(indice[candidate_idx]) >= new_indexes_maps.size())
				{
					continue;
				}

				int bias;
				lidar_iris_descriptor::featureDesc candidate_iris_feature =
					new_iris_features[indice[candidate_idx]];
				float candidate_distance = compare(iris_feature, candidate_iris_feature, &bias);

				if(candidate_distance < min_distance)
				{
					min_distance = candidate_distance;
					min_index = new_indexes_maps[indice[candidate_idx]];
					min_bias = bias;
				}
			}
		}
	...
	// threshold check
	if(min_index >= 0 && std::isfinite(min_distance) && min_distance < distance_threshold_)
	{
		result.first = min_index;
		result.second = min_bias;
		ROS_INFO("Iris inter candidate accepted: %d-%d <-> %d-%d, distance=%.3f threshold=%.3f bias=%d",
			iris_feature_index_pairs[cur_ptr].first, iris_feature_index_pairs[cur_ptr].second,
			iris_feature_index_pairs[min_index].first, iris_feature_index_pairs[min_index].second,
			min_distance, distance_threshold_, min_bias);
	...
	{
		ROS_INFO("Iris inter candidate not accepted: query=%d-%d best_index=%d distance=%.3f threshold=%.3f",
			iris_feature_index_pairs[cur_ptr].first, iris_feature_index_pairs[cur_ptr].second,
			min_index, min_distance, distance_threshold_);
		ROS_DEBUG("\033[1;33m[Iris Inter Not loop<%d>] query %d-%d. Best index:%d Dis:%.2f Bias:%d\033[0m", id_,
			iris_feature_index_pairs[cur_ptr].first, iris_feature_index_pairs[cur_ptr].second,
			min_index, min_distance, min_bias);
	}
	return result;
}
