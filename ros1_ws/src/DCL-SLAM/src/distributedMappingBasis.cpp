...
{
	world_to_odom_estimate_ = Pose3();
	// Robot a owns the world gauge.  Other robots must not claim that their
	// local odometry frame equals world before an inter-robot constraint exists.
	world_to_odom_valid_ = (id_ == 0);
		...
		if(it == id_)
		{
			...
			if(intra_robot_loop_closure_enable_ || inter_robot_loop_closure_enable_)
			{
				...
				robot.pub_descriptors = nh.advertise<dcl_slam::global_descriptor>(
					robot.name_+"/distributedMapping/globalDescriptors", 5, true);
	...
	else if(descriptor_type_num_ == DescriptorType::LidarIris)
	{
		keyframe_descriptor = unique_ptr<scan_descriptor>(new lidar_iris_descriptor(
			...
			exclude_recent_frame_num_, match_mode_, knn_candidates_,
			iris_min_valid_column_bins_, iris_min_valid_overlap_ratio_,
			4, 18, 1.6, 0.75,
	...
	loop_indexes.clear();
	proposed_inter_robot_loops_.clear();
	initialized_inter_robot_pairs_.clear();
	inter_robot_transform_candidates_.clear();
...
Pose3 distributedMapping::getWorldToOdomEstimate() const
{
	std::lock_guard<std::mutex> lock(world_to_odom_mutex_);
	return world_to_odom_estimate_;
}

bool distributedMapping::hasWorldToOdomEstimate() const
{
	std::lock_guard<std::mutex> lock(world_to_odom_mutex_);
	return world_to_odom_valid_;
}

std::string distributedMapping::getWorldFrame() const
{
	return world_frame_;
}
