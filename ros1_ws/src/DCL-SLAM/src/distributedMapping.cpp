...
namespace
{
bool ensurePcmOutputDirectory(std::string& error_message)
{
	const std::string& directory = global_map::GlobalMap::LOG_DIRECTORY;
	struct stat directory_status;
	if(::stat(directory.c_str(), &directory_status) != 0)
	{
		if(::mkdir(directory.c_str(), 0755) != 0 && errno != EEXIST)
		{
			error_message = "cannot create " + directory + ": " + std::strerror(errno);
			return false;
		}
	}
	else if(!S_ISDIR(directory_status.st_mode))
	{
		error_message = directory + " exists but is not a directory";
		return false;
	}

	// The bundled maximum-clique solver exchanges its graph through a file.
	// Probe actual write access before calling it: without this guard, a failed
	// write is read back as an empty graph and becomes reserve(size_t(-1)).
	const std::string probe_file = directory + ".pcm_write_probe_" +
		std::to_string(static_cast<long long>(::getpid()));
	std::ofstream probe(probe_file.c_str(), std::ios::out | std::ios::trunc);
	if(!probe.is_open())
	{
		error_message = "cannot write to " + directory;
		return false;
	}
	probe << "ok";
	probe.close();
	if(!probe)
	{
		error_message = "write test failed in " + directory;
		std::remove(probe_file.c_str());
		return false;
	}
	std::remove(probe_file.c_str());
	return true;
...
void distributedMapping::processPendingDescriptors()
{
	std::lock_guard<std::mutex> lock(descriptor_mutex_);
	while (!store_descriptors.empty())
	{
		const auto msg_data = store_descriptors.front().second;
		const auto msg_id = store_descriptors.front().first;
		store_descriptors.pop_front();
		keyframe_descriptor->saveDescriptorAndKey(
			msg_data.values.data(), msg_id, msg_data.index);
		ROS_INFO("Received global descriptor: robot=%d keyframe=%d",
			static_cast<int>(msg_id), msg_data.index);
	}
}
	...
	const ros::Time& timestamp)
{
	...
	// Normal motion-based keyframe selection.
	...
	{
		// A stationary robot would otherwise keep only keyframe 0 forever.  During
		// startup, collect a bounded number of time-spaced frames so descriptor
		// matching and geometric verification have multiple observations.
		if(startup_multi_frame_enable_ &&
			keyposes_cloud_6d->size() < static_cast<size_t>(startup_keyframe_count_))
		{
			const double elapsed = timestamp.toSec() - keyposes_cloud_6d->back().time;
			if(elapsed >= static_cast<double>(startup_keyframe_interval_))
			{
				ROS_INFO("Startup stationary keyframe accepted: robot=%d frame=%zu/%d elapsed=%.3fs",
					id_, keyposes_cloud_6d->size() + 1, startup_keyframe_count_, elapsed);
				return true;
			}
		}
		return false;
...
void distributedMapping::makeDescriptors()
{
	...
	const int current_key = static_cast<int>(initial_values->size()) - 1;
	pcl::PointCloud<PointPose3D>::Ptr descriptor_cloud(new pcl::PointCloud<PointPose3D>());
	int accumulated_frames = 1;

	// At stationary startup, use disjoint blocks expressed in the newest
	// keyframe coordinate system.  MID-360 needs temporal accumulation for
	// angular coverage, while non-overlapping blocks keep successive alignment
	// confirmations statistically meaningful.
	if(startup_multi_frame_enable_ && current_key < startup_keyframe_count_)
	{
		const int frames_in_current_block =
			(current_key + 1) % startup_accumulation_frames_;
		if(frames_in_current_block != 0)
		{
			ROS_INFO("Startup descriptor waiting for disjoint block: robot=%d "
				"stored=%d next_descriptor_at=%d",
				id_, current_key + 1,
				current_key + startup_accumulation_frames_ - frames_in_current_block + 1);
			return;
		}

		CHECK_LT(current_key, static_cast<int>(keyposes_cloud_6d->size()));
		CHECK_LT(current_key, static_cast<int>(robots[id_].keyframe_cloud_array.size()));
		const int first_key = current_key - startup_accumulation_frames_ + 1;
		const Pose3 current_pose = pclPointTogtsamPose3(keyposes_cloud_6d->points[current_key]);
		for(int key = first_key; key <= current_key; ++key)
		{
			CHECK_LT(key, static_cast<int>(keyposes_cloud_6d->size()));
			CHECK_LT(key, static_cast<int>(robots[id_].keyframe_cloud_array.size()));
			const Pose3 key_pose = pclPointTogtsamPose3(keyposes_cloud_6d->points[key]);
			const Pose3 key_to_current = current_pose.between(key_pose);
			*descriptor_cloud += *transformPointCloud(
				robots[id_].keyframe_cloud_array[key], key_to_current);
		}
		accumulated_frames = startup_accumulation_frames_;
	}
	else
	{
		*descriptor_cloud = *robots[id_].keyframe_cloud;
	}

	// downsample descriptor input
	cloud_for_decript_ds->clear();
	downsample_filter_for_descriptor.setInputCloud(descriptor_cloud);
	...
	{
		std::lock_guard<std::mutex> lock(descriptor_mutex_);
		descriptor_vec = keyframe_descriptor->makeAndSaveDescriptorAndKey(
			*cloud_for_decript_ds, id_, current_key);
	}
	...
	// keyfame index
	global_descriptor_msg.index = current_key;
	...
	ROS_INFO("Published global descriptor: robot=%d keyframe=%d accumulated_frames=%d points=%zu",
		id_, global_descriptor_msg.index, accumulated_frames, cloud_for_decript_ds->size());
}

void distributedMapping::publishPath()
{
	// publish path
	if(hasWorldToOdomEstimate() && pub_global_path.getNumSubscribers() != 0)
	{
...
void distributedMapping::publishTransformation(
	const ros::Time& timestamp)
{
	if(!hasWorldToOdomEstimate())
	{
		return;
	}

	static tf::TransformBroadcaster world_to_odom_tf_broadcaster;
	const Pose3 pose_between = getWorldToOdomEstimate();
...
void distributedMapping::outliersFiltering()
{
	...
	if(use_pcm_)
	{
		...
		auto acceptPreverifiedMeasurements = [this](
			const graph_utils::Transforms& transforms)
		{
			for(const auto& transform_entry : transforms.transforms)
			{
				const auto& transform = transform_entry.second;
				accepted_keys.insert(std::make_pair(transform.i, transform.j));
				if(Symbol(transform.i).chr() == char(id_ + 'a'))
				{
					other_robot_keys_for_optimization.insert(transform.j);
				}
				else
				{
					other_robot_keys_for_optimization.insert(transform.i);
				}
				measurements_accepted_num++;
			}
		};
		...
		for(const auto& neighbor : neighbors_within_communication_range)
		{
			...
			// Pairwise consistency needs at least two loop closures to form a
			// cycle.  A single startup constraint has already passed descriptor,
			// ICP and RANSAC validation, so it is vacuously PCM-consistent.  This
			// path also avoids asking PCM to construct odometry trajectories when
			// both stationary robots only contain keyframe 0.
			if(loop_closures_transforms.transforms.size() == 1)
			{
				const auto& transform = loop_closures_transforms.transforms.begin()->second;
				acceptPreverifiedMeasurements(loop_closures_transforms);
				ROS_INFO_THROTTLE(10.0,
					"PCM accepted the single startup loop: %c%zu <-> %c%zu",
					Symbol(transform.i).chr(), static_cast<size_t>(Symbol(transform.i).index()),
					Symbol(transform.j).chr(), static_cast<size_t>(Symbol(transform.j).index()));
				continue;
			}

			std::string pcm_output_error;
			if(!ensurePcmOutputDirectory(pcm_output_error))
			{
				// Every factor reaching here already passed descriptor, ICP, RANSAC,
				// and the new multi-observation transform consensus.  Keep mapping
				// alive if the optional PCM diagnostic-file path is unavailable.
				acceptPreverifiedMeasurements(loop_closures_transforms);
				ROS_ERROR_THROTTLE(5.0,
					"PCM file solver unavailable (%s); accepting %zu preverified inter-robot constraints",
					pcm_output_error.c_str(), loop_closures_transforms.transforms.size());
				continue;
			}
	...
	const bool& log_info)
{
	for(const auto& transform : robot_local_map_backup.getTransforms().transforms)
	{
		if(robot_local_map.getTransforms().transforms.find(transform.first) == robot_local_map.getTransforms().transforms.end())
		{
			...
			if(Symbol(factor.key1()).chr() != Symbol(factor.key2()).chr())
			{
				robot_local_map.robot_measurements::RobotMeasurements::addTransform(
					factor, transform.second.pose.covariance_matrix);
			}
			else
			{
...
void distributedMapping::updateWorldToOdomEstimate()
{
	const Symbol first_key(id_ + 'a', 0);
	if(!initial_values->exists(first_key))
	{
		return;
	}

	// initial_values contains the optimized world pose after DGS retraction,
	// while piror_odom stores the same keyframe in this robot's local odometry
	// frame.  Their composition is T_world_odom.
	const Pose3 first_pose_world = initial_values->at<Pose3>(first_key);
	const Pose3 first_pose_odom = robots[id_].piror_odom;
	const Pose3 world_to_odom = first_pose_world * first_pose_odom.inverse();

	{
		std::lock_guard<std::mutex> lock(world_to_odom_mutex_);
		// A non-anchor optimizer can finish a locally gauged cycle before the
		// accepted startup edge reaches this process.  Such an identity result is
		// not a world alignment and must not prevent the verified edge from being
		// latched later.
		if(id_ != 0 && !world_to_odom_valid_)
		{
			return;
		}
		if(world_to_odom_valid_ && id_ != 0)
		{
			const Pose3 delta = world_to_odom_estimate_.between(world_to_odom);
			const double translation_jump = delta.translation().norm();
			const double rotation_jump = Rot3::Logmap(delta.rotation()).norm();
			if(translation_jump > world_to_odom_max_translation_jump_ ||
				rotation_jump > world_to_odom_max_rotation_jump_)
			{
				ROS_WARN("Rejected world->%s optimizer jump: delta=[%.3fm %.3frad] "
					"candidate_xyz=[%.3f %.3f %.3f]; keeping latched transform",
					robots[id_].odom_frame_.c_str(), translation_jump, rotation_jump,
					world_to_odom.translation().x(), world_to_odom.translation().y(),
					world_to_odom.translation().z());
				return;
			}
		}
		world_to_odom_estimate_ = world_to_odom;
		world_to_odom_valid_ = true;
	}

	LOG(INFO) << "worldToOdom:[" << id_ << "] "
		<< world_to_odom.translation().x() << " "
		<< world_to_odom.translation().y() << " "
		<< world_to_odom.translation().z() << " "
		<< world_to_odom.rotation().roll() << " "
		<< world_to_odom.rotation().pitch() << " "
		<< world_to_odom.rotation().yaw() << "." << endl;
	ROS_INFO("world->%s updated: xyz=[%.3f %.3f %.3f], rpy=[%.3f %.3f %.3f]",
		robots[id_].odom_frame_.c_str(),
		world_to_odom.translation().x(), world_to_odom.translation().y(),
		world_to_odom.translation().z(), world_to_odom.rotation().roll(),
		world_to_odom.rotation().pitch(), world_to_odom.rotation().yaw());
}

void distributedMapping::latchStartupWorldToOdom(
	const dcl_slam::loop_info& inter_loop,
	const Pose3& pose_between)
{
	// The global gauge is owned by robot a.  For its direct startup edge to
	// another robot, the verified measurement already determines T_world_odom:
	// T_odom0_body0 * T_body0_body1 * T_body1_odom1.
	if(inter_loop.robot0 != 0 || id_ != inter_loop.robot1)
	{
		return;
	}
	...
	const Pose3 world_to_odom = pose0 * pose_between * pose1.inverse();
	{
		std::lock_guard<std::mutex> lock(world_to_odom_mutex_);
		if(world_to_odom_valid_)
		{
			return;
		}
		world_to_odom_estimate_ = world_to_odom;
	...
	ROS_INFO("Latched startup world->%s directly from confirmed alignment: "
		"xyz=[%.3f %.3f %.3f] rpy=[%.3f %.3f %.3f]",
		robots[id_].odom_frame_.c_str(), world_to_odom.translation().x(),
		world_to_odom.translation().y(), world_to_odom.translation().z(),
		world_to_odom.rotation().roll(), world_to_odom.rotation().pitch(),
		world_to_odom.rotation().yaw());
}
