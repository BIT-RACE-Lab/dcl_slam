...
class lidar_iris_descriptor : public scan_descriptor
{
	public:
		struct featureDesc
		{
			...
			cv::Mat1b T;
			cv::Mat1b M;
			cv::Mat1b V; // valid azimuth columns; invalid/no-return sectors are masked
		};
	...
	private:
		...
		int match_mode_;
		int candidates_num_;
		int min_valid_column_bins_;
		float min_valid_overlap_ratio_;
